...
目前对于外提供 WebSocket API 开发接口， 供开发者获取行情数据和进行交易操作。
*请注意行情和交易是两个不同服务器*，行情接口无需认证可以自由访问，交易部分则需要用户开通 API-KEY 后通过自己的 KEY 认证授权后方可使用。

官方的生产环境：

```txt
行情服务： wss://www.host2api.com/v1/market
交易服务： wss://www.host2api.com/v1/trade
```
...
// 收到推送消息
{
    ...
    "data":{
        ...
        "FundingShortR":0,            // 当前未使用字段
        "Tz8High24": 9553.5,          //24小时最高价，区间为每日00:00-24:00（UTC+8）
        "Tz8Low24": 9416.5,           //24小时最低价，区间为每日00:00-24:00（UTC+8）
        "Tz8Prz24": 9466.5,           //24小时初始价格，区间为每日00:00-24:00（UTC+8）
        "Tz8Turnover24": 821465132.869999,   //24小时持仓量，区间为每日00:00-24:00（UTC+8）
        "Tz8Volume24": 8647130        //24小时量，区间为每日00:00-24:00（UTC+8）
    }
}
...
如需恢复正常，则增加"\_\_fast\_\_"即可切换回正常模式。
 ...
 * 例:
 *  UserName: "xexample@gmail.com",
 *  UserCred: "mVAAADjNHzhvehaEvU$BMJoU7BZk"
 *  SecretKey: "uLgAAHMw62di3hUPypuETMWGzHx852swxM7V0b2HObba5gYNNrLkuvQ4I"
 *  Req: "Login"
 *  rid: 1
 * 根据上面的信息可以生成如下签名:
 * signature = md5("Login"+"1"+ JSON.stringify({UserName:"example@gmail.com",UserCred:"mVAAADjNHzhvehaEvU$BMJoU7BZk"}) +"1538222696758" + "uLgAAHMw62di3hUPypuETMWGzHx852swxM7V0b2HObba5gYNNrLkuvQ4I")
 ...
 *  "args":{                    			// 服务端所需的参数
 *      "UserName":"example@gmail.com",              	                                // 账号
 *      "UserCred":"FQAAE51mDakz1MShjRihILiLuNi32uxMNjUjc0MOs2PUdhewc1gru9kyCo8="   	// token
 *      "AuthType":2
 *  },
 *  "signature": "888a7a0e94f5eb6399995afd59958a9f" 	// 签名,可参考生产签名的方法
 * }
 *
 * token登录生成签名的方法:
 ...
 *  rid: 1
 * 根据上面的信息可以生成如下签名:
 * signature = md5("Login"+"1"+ JSON.stringify({UserName:"example@gmail.com",UserCred:"mVAAADjNHzhvehaEvU$BMJoU7BZk",AuthType:2}) +"1538222696758")
 * signature结果为:"888a7a0e94f5eb6399995afd59958a9f"
 ***/
...
查询结果的数据结构详见[交易相关数据结构定义](#交易相关数据结构定义)
...
// 收到返回消息
{
    ...
    "data":[
        ...
        {"UId":"1234567","AId":"123456701","Coin":"USDT","Depo":5,"WDrw":0,"PNL":0,"Frz":0,"Status":2}
    ]
}
```
